...
function detectDevToolsBrowser() {
...
function causePerformanceViolation() {
...
function demonstrateMessageSources() {
...
function filterByRegexExamples() {
...
function getFamiliarSourcesUIExample() {
...
function variableInspectionExample() {
    // These variables can be inspected in the Scope pane when breakpoint is hit
    const petName = petInfo.name;
    const petWeight = petInfo.weight;
    const petHappiness = petInfo.happiness;
    const isHappy = petHappiness > 5;
...
function watchExpressionsExample() {
    let counter = 0;
    const max = 5;
...
function pauseOnExceptionExample() {
    ...
    function problematicFunction() {
        ...
        const result = petName.nonexistentProperty.getValue();
    }
    try {
        problematicFunction();
    }
...
function pauseWithDebuggerExample() {
...
function stepThroughCodeExample() {
